  ...
  try {
    ...
    if (instrumento.dataValues.Raitings.length) {
      for (let i = 0; i < instrumento.dataValues.Raitings.length; i++) {
        SumaCalificaciones =
          SumaCalificaciones + instrumento.dataValues.Raitings[i].star;
      }
      let final = parseInt(
        SumaCalificaciones / instrumento.dataValues.Raitings.length
      );
...
async function add_raiting(req, res) {
  try {
    ...
    for (let i = 0; i < instrumentoCalificado.dataValues.Raitings.length; i++) {
      if (
        instrumentoCalificado.dataValues.Raitings[i].dataValues.userId ==
        user_id
      ) {
    ...
    let creado = await Raiting.create({
